...
def load_env() -> None:
    ...
    1. 既に process env に存在するもの (launchd 経由) — 最優先
    2. ローカル `.env` ファイル (dev 用 fallback)
    3. GCP Secret Manager (`X_USE_SECRET_MANAGER` truthy 時のみ、デフォルト on)

    Secret Manager mode では `X_ACCOUNT` と `GOOGLE_CLOUD_PROJECT` を見て
    `xmcp-app-credentials` + `xmcp-user-{account}` を fetch して os.environ に注入。
    """
    ...
    if env_path.exists():
        try:
            ...
            pass
    if not is_truthy(os.getenv("X_USE_SECRET_MANAGER", "1")):
        return
    from secret_loader import load_secrets_from_gcp

    project_id = os.environ.get("GOOGLE_CLOUD_PROJECT", "ryan-self-management")
    account = os.environ.get("X_ACCOUNT", "").strip().lower()
    if not account:
        raise RuntimeError(
            "X_ACCOUNT env var required when X_USE_SECRET_MANAGER=1 "
            "(set to 'ryantsuji' or 'ryanaircloset')"
        )
    load_secrets_from_gcp(project_id, account)
