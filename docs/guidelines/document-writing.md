...
- **`docs/observability/`**: per-app observability の経路 overview と「データが来ない時」の debug runbook (例: `docs/observability/ryantsuji-dev-web.md`)。配線設計詳細はコード側 JSDoc が SSoT、本ディレクトリは外部システム (Tempo / Loki / BQ console / browser devtools) から見る人間用 runbook
