...
cortex が「会社の知識を外部化する」 system なら、self-management は「**個人 (Ryan) の知識・思想・運用を外部化する**」 system である。
...
cortex と同じ哲学を、scope を「Ryan というシステム」に絞って適用する。
...
cortex の Product Graph が「コード×DB×ドキュメント×インフラ」を統合するなら、ryan-product-graph は「**Ryan の人格・思想・content・関係性**」を統合する。
...
## 3. AI を信じない、仕組みで守る (cortex 哲学の適用)
...
| 軸 | cortex | self-management |
|----|--------|-----------------|
| scope | 会社 (airCloset) のコード+DB+運用 | Ryan 個人の content+思想+engagement |
| ノード規模 | ~8,000 (現在) → 数万 | 数百 → 数千 |
| 主用途 | エンジニアリング自律化 | 個人発信 + 思想保管 |
| AI 出力先 | コード / 業務自動化 | 投稿 / 記事 / 返信 |
| voice 課題 | small (機械的タスク) | large (個人 brand) |
| 公開範囲 | 社内のみ | content は public、graph は private |

cortex で得た学びを personal scale で再利用するのが本リポジトリの role。
...
### 6.3 cross-cortex 接続 (慎重に)

cortex (会社) と self-management (個人) は別 GCP プロジェクトで物理分離。但し公開済み content (Zenn/dev.to/X) を引用 / リンクする形での疎結合は許容 (cortex 側からも「Ryan の発信」を参照する形)。

---

## 7. 設計原則

cortex の VISION から借りる:
