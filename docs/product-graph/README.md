...
Ryan の人格・content・思想・関係性 + self-management 自身のコード/インフラを統合する **3 graph 構造**。cortex の `cortex-product-graph` パターンを個人 scope に降ろした形だが、目的別に 3 つのサブグラフに分離。
...
### cortex-product-graph との関係

| | cortex | ryan-product-graph (本リポジトリ) |
|---|---|---|
| scope | 会社 (airCloset) | 個人 (Ryan) |
| BQ dataset | `cortex` (会社プロジェクト) | `ryan` (`ryan-self-management` プロジェクト) |
| graph 数 | 1 (統合) | **3** (product / release-note / personal) |
| MCP server | `mcp-cortex-product-graph` | (将来) `mcp-ryan-product-graph` |
...
self-management mono-repo 自身の technical structure。cortex の product-graph と同型 (db-graph 含む)。
...
- **product-graph**: cortex と同じ `1 nodes table + node_type discriminator` style。新 type が頻出する見込み (Function/Module/...が増える) ため拡張性重視。
...
| **P5** | code @graph-* タグ + parser で product-graph を自動生成 (cortex 同型) |
| **P6** | MCP server (Cloud Run) を立てる、`.mcp.json` に登録 |
| **P7** | voice retrieval / corpus 学習 → 自律 draft 生成 |

## 関連 doc

- [cortex の Product Graph 設計](../../../cortex/docs/product-graph/README.md) — originator pattern
