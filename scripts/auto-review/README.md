...
| merge | 直近の verdict コメントが `<!-- VERDICT:APPROVE -->` + 当該 head_sha 未 merge + CI 全 green | `gh pr merge --squash --delete-branch` |
| index | `origin/main` の SHA が前回 index 時点から動いた | detached process で `pnpm graph:build` を fire-and-forget kick (5-30 分かかるので poll loop は block しない) |
