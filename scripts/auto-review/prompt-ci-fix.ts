...
export function buildCiFixPrompt(input: CiFixPromptInput): string {
  ...
  return [
    ...
    "pnpm test:coverage",
    "```",
    "なお `infra/*` 配下の Pulumi preview は repo 内のローカル 6 gate に含まれない。Pulumi 失敗は GH Actions log を読んで diagnostic、`pulumi/` ディレクトリでローカル `pulumi preview` を実行できるなら回す。secret 不足など bot ローカルで再現不可能な失敗は FIX_FAILED 妥当。",
    "",
    ...
    "# 失敗時の挙動",
    "",
    "- 診断はしたが AI 判断では fix できない / 失敗が一過性で repo の変更で直らない / 6 gate を green にできない場合、push せず終了。stdout に `<!-- FIX_FAILED:<理由> -->` を 1 行残す。呼び出し側 (ci-fix-job.ts) が当該 SHA を bookmark + iteration counter を +1 進めて、同 SHA の永久 retry を遮断する (MAX_ITERATIONS_PER_PR cap で必ず止まる)。安易に early-give-up しないこと、限界まで try してから FIX_FAILED を出すこと。",
  ].join("\n");
}
